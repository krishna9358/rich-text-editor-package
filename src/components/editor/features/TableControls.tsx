    ...
    <div>
      ...
      {open && (
        <div className="fixed z-50 bg-white border rounded shadow p-2 text-sm" style={{ top: buttonPos.top + 24, left: buttonPos.left - 160 }}>
          {/* Add row below */}
          <button className="block flex items-center gap-2 w-full text-left px-2 py-1 hover:bg-gray-100" onClick={() => run(() => editor.chain().focus().addRowAfter().run())}>
            <PlusCircleIcon className="w-4 h-4" />
            Add row below
          </button>

          {/* Add row above */}
          <button className="block flex items-center gap-2 w-full text-left px-2 py-1 hover:bg-gray-100" onClick={() => run(() => editor.chain().focus().addRowBefore().run())}>
            <PlusCircleIcon className="w-4 h-4" />
            Add row above
          </button>

          {/* Add column right */}
          <button className="block flex items-center gap-2 w-full text-left px-2 py-1 hover:bg-gray-100" onClick={() => run(() => editor.chain().focus().addColumnAfter().run())}>
            <PlusCircleIcon className="w-4 h-4" />
            Add column right
          </button>

          {/* Add column left */}
          <button className="block flex items-center gap-2 w-full text-left px-2 py-1 hover:bg-gray-100" onClick={() => run(() => editor.chain().focus().addColumnBefore().run())}>
            <PlusCircleIcon className="w-4 h-4" />
            Add column left
          </button>
        </div>
